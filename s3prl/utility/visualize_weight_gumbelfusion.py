...
def visualize_weight_gumbelfusion(featurizer):
    weights = featurizer.get('weights')
    if weights is not None:
        ...
        for i, count in enumerate(counts):
            print(f"\tLayer {i:>2}: {count.item():>3}")
        ...
        print(f'Min anneal ratio: {stds.min().item(): 0.4f} at dim {stds.argmin().item()}')
...
if __name__ == '__main__':
    ...
    counts = visualize_weight_gumbelfusion(ckpt['Featurizer'])
