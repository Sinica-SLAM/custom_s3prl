...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir, **kwargs):
        ...
        self.dictionary = Dictionary.load(self.datarc.get(
            "dict_path", str(Path(__file__).parent / "char.dict")))
    ...
    def get_dataloader(self, split):
        ...
        if not hasattr(self, f'{split}_dataset'):
            ...
            setattr(self, f'{split}_dataset', SequenceDataset(
    ...
    def log_records(self, split, records, logger, global_step, batch_ids, total_batch_num, **kwargs):
        ...
        if 'test' in split or 'dev' in split:
            ...
            hyp_ark = open(os.path.join(
                self.expdir, f'{split}-{lm}-hyp.ark'), 'w')
            ref_ark = open(os.path.join(
                self.expdir, f'{split}-{lm}-ref.ark'), 'w')
