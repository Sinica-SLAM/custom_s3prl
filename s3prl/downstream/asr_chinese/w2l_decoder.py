    ...
    def __init__(self, args, tgt_dict):
        ...
        if args.lexicon:
            ...
            self.decoder_opts = LexiconDecoderOptions(
                beam_size=args.beam,
                beam_size_token=int(
                    getattr(args, "beam_size_token", len(tgt_dict))),
...
class W2lFairseqLMDecoder(W2lDecoder):
    ...
    def decode(self, emissions):
        ...
        def make_hypo(result):
            hypo = {"tokens": self.get_tokens(
                result.tokens), "score": result.score}
            if self.lexicon:
                hypo["words"] = [idx_to_word(x)
