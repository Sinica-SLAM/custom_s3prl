...
class RNNs(nn.Module):
    def __init__(self,
        ...
        self.sample_rate = 1 if total_rate == - \
            1 else round(total_rate / upstream_rate)
    ...
    def forward(self, x, x_len):
        ...
        if self.sample_rate > 1:
            x, x_len = downsample(
                x, x_len, self.sample_rate, self.sample_style)
...
class Wav2Letter(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, upstream_rate, total_rate=320, **kwargs):
        ...
        self.acoustic_model = nn.Sequential(
            nn.Conv1d(in_channels=input_dim, out_channels=250,
                      kernel_size=48, stride=first_stride, padding=23),
            ...
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=2000, out_channels=output_dim,
                      kernel_size=1, stride=1, padding=0),
            nn.ReLU(inplace=True)
        )
