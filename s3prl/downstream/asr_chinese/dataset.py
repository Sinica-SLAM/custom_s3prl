...
class SequenceDataset(Dataset):
    ...
    def __init__(self, split, bucket_size, dictionary, libri_root, bucket_file, **kwargs):
        ...
        assert os.path.isdir(
            bucket_file), 'Please first run `python3 preprocess/generate_len_for_bucket.py -h` to get bucket file.'
    ...
    def __getitem__(self, index):
        ...
        wav_batch = [self._load_wav(x_file).numpy()
                     for x_file in self.X[index]]
        label_batch = [self.Y[self._parse_x_name(
            x_file)].numpy() for x_file in self.X[index]]
